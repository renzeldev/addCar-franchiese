...
@Injectable()
export class SubFranchiseeStatementResolverService implements Resolve<StatementViewModel> {
  ...
  resolve(route: ActivatedRouteSnapshot): StatementViewModel | Observable<StatementViewModel> | Observable<never> {
    if (route.params['uid']) {
      ...
      return this.statementService.getSubfranchiseeStatement(route.params['uid']).pipe(catchError(() => {
