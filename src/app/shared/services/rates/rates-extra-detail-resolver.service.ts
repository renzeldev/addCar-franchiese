...
@Injectable()
export class RatesExtraDetailResolverService implements Resolve<ExtrasListModel> {
  constructor(
  ...
  ): ExtrasListModel | Observable<ExtrasListModel> | Observable<never> {
    if (route.params['uid'] && route.params['uid'] !== 'new') {
      this.spinnerService.show();
      return this.service.getExtraDetail(route.params['uid']).pipe(
        catchError(() => {
      ...
      if(route.params['uid'] === 'new') {
        return of(new ExtrasListModel());
      }
    }
  }
}
